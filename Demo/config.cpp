...
{
	return gBuffer->config(renderer->getWidth(), renderer->getHeight(), renderer->getD3DDevice(), renderer->getContext());
}
...
HRESULT configModels(void)
{
	...
	XMFLOAT4 mat_ambient(0.2f, 0.2f, 0.2f, 1.0f);
	XMFLOAT4 mat_diffuse(0.8f, 0.8f, 0.8f, 1.0f);
	...
	hr = sol->config(gObjectProgram, renderer->getD3DDevice(), renderer->getContext());
	if (FAILED(hr))
		return hr;
	...
	// Heli
	hr = heli->config(gObjectProgram, renderer->getD3DDevice(), renderer->getContext());
	if (FAILED(hr))
		return hr;
...
{
	return screen->config(screenProgram, renderer->getD3DDevice(), renderer->getContext());
}
...
HRESULT configSkyBox(void)
{
	...
	hr = skybox->load("resources/Skybox/rightred2.jpg", "resources/Skybox/leftred2.jpg",
		...
		10, 
		gSkyboxProgram, renderer->getD3DDevice(), renderer->getContext());
	if (FAILED(hr))
		return hr;
